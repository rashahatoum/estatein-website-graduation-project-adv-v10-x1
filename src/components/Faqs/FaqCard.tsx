  ...
  return (
    <div className="Faq-Card flex h-full flex-col justify-between gap-20 rounded-xl border border-grey-15 bg-grey-08 p-30 md:p-40">
      <div className="Faq-Content flex flex-col gap-16">
        <h3 className="text-[18px] md:text-[24px] font-semibold text-white">
          {faq.question}
        </h3>
        <p className="text-[14px] md:text-[16px] text-grey-60">
          {faq.answer}
        </p>
      </div>

      <button className="w-full md:w-fit rounded-lg border border-grey-15 bg-grey-10 px-20 py-10 text-[14px] font-medium text-white transition-colors hover:bg-grey-15 cursor-pointer">
        Read More
      </button>
    </div>
  );
